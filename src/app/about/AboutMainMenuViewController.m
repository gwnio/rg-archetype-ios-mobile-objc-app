...
@interface AboutMainMenuViewController () <UITableViewDelegate, UITableViewDataSource>

@end

@implementation AboutMainMenuViewController {
...
- (void) loadView {
	...
	[_items addObject:[[MenuTableViewCellItem alloc] init:[NSNumber numberWithInt:AboutMainMenuItemName] :@"Name" :@""]];
	[_items addObject:[[MenuTableViewCellItem alloc] init:[NSNumber numberWithInt:AboutMainMenuItemPhone] :@"Phone #" :@""]];
	[_items addObject:[[MenuTableViewCellItem alloc] init:[NSNumber numberWithInt:AboutMainMenuItemFavoriteColors] :@"Favorite Colors" :@""]];
...
- (void)selectItem:(AboutMainMenuItem)item
{
	if ([_items count] > 0) {
		for (int i = 0; i < _items.count; i++) {
			MenuTableViewCellItem *cellItem = (MenuTableViewCellItem*)_items[i];
